...
RUN \
    ...
    && DECRED_ARCHIVE="decrediton-$DECRED_VERSION.tar.gz" \ 
    && MANIFEST_SIGN="manifest-decrediton-$DECRED_VERSION.txt.asc" \
    && MANIFEST="manifest-decrediton-$DECRED_VERSION.txt" \
    && cd /tmp \
